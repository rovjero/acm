...
public class SpringSecurityConfig2 extends WebSecurityConfigurerAdapter {
    ...
    public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
		...
		final String findRoles = "select username,role " + "from user_roles "
				+ "where username = ?";
		
    	auth.jdbcAuthentication().dataSource(dataSource()).usersByUsernameQuery(findUserQuery).authoritiesByUsernameQuery(findRoles);
    	/*
